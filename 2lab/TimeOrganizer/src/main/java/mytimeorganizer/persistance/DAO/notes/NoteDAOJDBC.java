...
public class NoteDAOJDBC implements NoteDAO {
    ...
    public void addNote(Note note) {
        ...
        try (
            ...
            if (affectedRows == 0) {
                throw new DAOException("Creating note failed, no rows affected.");
            }
            try (ResultSet generatedKeys = statement.getGeneratedKeys()) {
                if (generatedKeys.next()) {
                    note.setId(generatedKeys.getLong(1));
                } else {
                    throw new DAOException("Creating note failed, no generated key obtained.");
                }
            }
        } catch (SQLException e) {
            throw new DAOException(e);
    ...
    public void updateNoteDescription(String description, Long userId, LocalDate date) {
        try (
            ...
            int affectedRows = statement.executeUpdate();
            if (affectedRows == 0) {
                throw new DAOException("Changing note failed, no rows affected.");
            }
        } catch (SQLException e) {
            throw new DAOException(e);
        }
